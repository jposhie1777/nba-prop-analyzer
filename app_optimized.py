...
def load_props_cached(_bq_client, query):
    ...
    return add_dynamic_averages(df)
...
# player selector
players_today = ["All players"] + sorted(props_df["player"].dropna().unique().tolist())
...
# bookmaker filter
books_available = sorted(props_df["bookmaker"].dropna().unique().tolist())
...
sel_odds_range = st.sidebar.slider("American odds range", int(odds_arr.min()), int(odds_arr.max()), (int(odds_arr.min()), int(odds_arr.max())))
...
# ------------------------------------------------------
# MEMORY-OPTIMIZED PROPS TABLE BUILDER
# ------------------------------------------------------
...
def build_props_table(df, game_pick, player_pick, stat_pick, books, odds_range, min_ev, min_hit10):
    ...
    # ONE SINGLE FINAL COPY (memory optimized)
    df_display = view[[
        "player", "market", "line", "price", "bookmaker",
        "hit5_frac", "hit10_frac", "hit20_frac",
...
def get_player_game_log(df, player, market):
    ...
    s = s.dropna(subset=["game_date"]).sort_values("game_date")
    return s.tail(20)
...
with tab2:
    ...
    if game_logs_df.empty:
        ...
        players = ["(choose)"] + sorted(props_df["player"].dropna().unique().tolist())
        p_pick = st.selectbox("Player", players)
        if p_pick == "(choose)":
            st.stop()

        markets = sorted(props_df[props_df["player"] == p_pick]["market"].unique().tolist())
        if not markets:
        ...
        lines = sorted(props_df[(props_df["player"] == p_pick) &
                                (props_df["market"] == stat_pick)]["line"].dropna().unique().tolist())
        if not lines:
