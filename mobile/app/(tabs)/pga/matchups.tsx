import { ScrollView, View, Text, ActivityIndicator } from "react-native";
import { Stack } from "expo-router";
import { useEffect, useMemo, useState } from "react";

import { useTheme } from "@/store/useTheme";
import { usePgaPlayers } from "@/hooks/pga/usePgaPlayers";
import { usePgaQuery } from "@/hooks/pga/usePgaQuery";
import { MetricCard } from "@/components/pga/MetricCard";
import { SearchPicker, SearchItem } from "@/components/pga/SearchPicker";
import { PgaMatchupResponse } from "@/types/pga";

export default function MatchupsScreen() {
  const { colors } = useTheme();
  const [searchA, setSearchA] = useState("");
  const [searchB, setSearchB] = useState("");
  const [playerA, setPlayerA] = useState<SearchItem | null>(null);
  const [playerB, setPlayerB] = useState<SearchItem | null>(null);

  const { data: playersA } = usePgaPlayers({ search: searchA });
  const { data: playersB } = usePgaPlayers({ search: searchB });

  const itemsA = useMemo(
    () =>
      (playersA?.data || []).map((player) => ({
        id: player.id,
        label: player.display_name,
        subLabel: player.country ?? "",
      })),
    [playersA]
  );

  const itemsB = useMemo(
    () =>
      (playersB?.data || []).map((player) => ({
        id: player.id,
        label: player.display_name,
        subLabel: player.country ?? "",
      })),
    [playersB]
  );

  useEffect(() => {
    if (!playerA && itemsA.length > 0) {
      setPlayerA(itemsA[0]);
    }
  }, [itemsA, playerA]);

  useEffect(() => {
    if (!playerB && itemsB.length > 0) {
      setPlayerB(itemsB[0]);
    }
  }, [itemsB, playerB]);

  const { data, loading, error } = usePgaQuery<PgaMatchupResponse>(
    "/pga/analytics/matchup",
    {
      player_id: playerA?.id,
      opponent_id: playerB?.id,
    },
    !!playerA && !!playerB
  );

  return (
    <>
      <Stack.Screen
        options={{
          title: "Matchup Ratings",
          headerStyle: { backgroundColor: colors.surface.screen },
          headerTintColor: colors.text.primary,
        }}
      />
      <ScrollView
        style={{ flex: 1, backgroundColor: colors.surface.screen }}
        contentContainerStyle={{ padding: 16, paddingBottom: 40 }}
      >
        <SearchPicker
          title="Player A"
          placeholder="Search players..."
          query={searchA}
          onQueryChange={setSearchA}
          items={itemsA}
          selectedId={playerA?.id}
          onSelect={setPlayerA}
        />
        <SearchPicker
          title="Player B"
          placeholder="Search players..."
          query={searchB}
          onQueryChange={setSearchB}
          items={itemsB}
          selectedId={playerB?.id}
          onSelect={setPlayerB}
        />

        {loading ? (
          <View style={{ padding: 20, alignItems: "center" }}>
            <ActivityIndicator color={colors.accent.primary} />
          </View>
        ) : null}

        {error ? (
          <Text style={{ color: colors.text.danger, marginTop: 12 }}>
            {error}
          </Text>
        ) : null}

        {data ? (
          <MetricCard
            title="Head-to-Head Summary"
            subtitle="Shared tournaments in selected season"
            metrics={[
              { label: "Starts", value: data.starts },
              { label: "Wins", value: data.wins },
              { label: "Losses", value: data.losses },
              { label: "Ties", value: data.ties },
              { label: "Win Rate", value: data.win_rate },
            ]}
          />
        ) : null}

        {(data?.matches || []).slice(0, 10).map((match, index) => (
          <MetricCard
            key={`${match.tournament?.id ?? index}`}
            title={match.tournament?.name ?? "Tournament"}
            subtitle={match.tournament?.start_date ?? ""}
            metrics={[
              { label: "Player A", value: match.player_position },
              { label: "Player B", value: match.opponent_position },
            ]}
          />
        ))}
      </ScrollView>
    </>
  );
}
