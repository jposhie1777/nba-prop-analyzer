  ...
  return (
    <ScrollView style={s.screen} contentContainerStyle={s.content}>
      ...
      {(data?.pitchers ?? []).map((pitcher) => {
        const tone = gradeTone(pitcher.over_grade);
        const standardKey = makeSlipId(pitcher);
          ...
          <View key={String(pitcher.pitcher_id)} style={[s.panel, { borderColor: colors.border.subtle }]}>
            ...
            <View style={s.pitcherHeader}>
              ...
              <View style={[s.gradePill, { borderColor: tone.border, backgroundColor: tone.bg }]}>
                <Text style={[s.gradeText, { color: tone.text }]}>
                  {pitcher.over_grade ?? "—"} • {pitcher.lean ?? "—"}
                </Text>
              </View>
            </View>
            ...
            <View style={s.projRow}>
              <View style={s.projBox}>
                <Text style={s.projLabel}>K LINE</Text>
                <Text style={s.projValue}>{fmt(pitcher.k_line, 1)}</Text>
              </View>
              <View style={s.projBox}>
                <Text style={s.projLabel}>PROJ Ks</Text>
                <Text style={[s.projValue, { color: "#22D3EE" }]}>{fmt(pitcher.proj_ks, 1)}</Text>
              </View>
              <View style={s.projBox}>
                <Text style={s.projLabel}>EDGE</Text>
                <Text style={[s.projValue, { color: leanColor(pitcher.lean) }]}>
                  {pitcher.edge != null ? (pitcher.edge > 0 ? `+${pitcher.edge}` : `${pitcher.edge}`) : "—"}
                </Text>
              </View>
              <View style={s.projBox}>
                <Text style={s.projLabel}>SIGNAL</Text>
                <Text style={s.projValue}>{fmt(pitcher.k_signal_score, 1)}</Text>
              </View>
              <View style={s.projBox}>
                <Text style={s.projLabel}>CONF</Text>
                <Text style={[s.projValue, { color: confidenceColor(pitcher.confidence) }]}>
                  {pitcher.confidence ?? "—"}
                </Text>
              </View>
            </View>
            ...
            <View style={s.statsCard}>
              <Text style={s.statsTitle}>Pitcher K Profile</Text>
            ...
            {/* Arsenal */}
            <View style={s.statsCard}>
              <Text style={s.statsTitle}>Arsenal</Text>
              <View style={s.statsGrid}>
            ...
            {pitcher.hand_split ? (
              <View style={s.statsCard}>
                <Text style={s.statsTitle}>Split: {pitcher.hand_split}</Text>
                <View style={s.statsGrid}>
            ...
            {oppK?.team_name ? (
              <View style={s.statsCard}>
                <Text style={s.statsTitle}>Opp K Vulnerability: {oppK.team_name}</Text>
                <View style={s.statsGrid}>
            ...
            {/* Hit rates */}
            <View style={s.statsCard}>
              <Text style={s.statsTitle}>Hit Rates & Averages</Text>
              <View style={s.statsGrid}>
            ...
            {/* Standard line bet buttons */}
            <View style={s.betSection}>
              <Text style={s.betLabel}>
                Standard: Over {fmt(pitcher.k_line, 1)} Ks {fmtOdds(pitcher.k_best_price)} ({pitcher.k_best_book ?? "—"})
              </Text>
              <View style={s.betRow}>
            ...
            {(pitcher.alt_lines ?? []).length > 0 ? (
              <AltLinesSection pitcher={pitcher} selectedKeys={selectedKeys} togglePitcher={togglePitcher} openUrl={openUrl} getDkUrl={getDkUrl} getFdUrl={getFdUrl} addKPropToSlip={addKPropToSlip} openBetslip={openBetslip} />
            ) : null}
          </View>
        );
      })}
    ...
    <View style={s.altSection}>
      <Pressable onPress={() => setExpanded(!expanded)} style={s.altToggle}>
        <Text style={s.altToggleText}>
          Alt Lines ({(pitcher.alt_lines ?? []).length}) {expanded ? "▲" : "▼"}
        </Text>
      </Pressable>
      {expanded ? (
        <View style={s.altBody}>
          {(pitcher.alt_lines ?? []).map((alt) => {
            ...
            const altKey = `mlb-k-${pitcher.pitcher_id}-${lineVal}`;
...
const s = StyleSheet.create({
  ...
  // Stats card
  statsCard: { borderWidth: StyleSheet.hairlineWidth, borderColor: "#374151", borderRadius: 10, backgroundColor: "#111827", padding: 10, gap: 6 },
  statsTitle: { color: "#94A3B8", fontSize: 11, fontWeight: "800" },
