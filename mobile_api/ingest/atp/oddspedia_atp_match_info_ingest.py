...
def _ensure_table(client: bigquery.Client, table: str, schema: List[bigquery.SchemaField]) -> None:
...
def _truncate_and_insert(client: bigquery.Client, table: str, rows: List[Dict[str, Any]]) -> int:
    table_id = _full_table_id(client, table)
...
def _build_betting_rows(match_id: int, mk: Optional[int], data: Dict[str, Any], betting_stats: Dict[str, Any], ingested_at: str, scraped_date: str) -> List[Dict[str, Any]]:
...
def _build_per_match_rows(match_id: int, mk: Optional[int], data: Dict[str, Any], per_match: Dict[str, Any], ingested_at: str, scraped_date: str) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    base = _base_row(match_id, mk, data, ingested_at, scraped_date)
...
def ingest_match_info(url: Optional[str] = None, *, dry_run: bool = False, today_only: bool = True) -> Dict[str, Any]:
    ...
    for match in matches:
        ...
        if bs:
            betting_rows.extend(_build_betting_rows(mid, mk, data, bs, ingested_at, scraped_date))

        pms = match.get("per_match_stats") or {}
        if pms:
            per_match_rows.extend(_build_per_match_rows(mid, mk, data, pms, ingested_at, scraped_date))
        ...
        for side_key, side_name in (("last_matches_home", "home"), ("last_matches_away", "away")):
                ...
                for p in (block.get(status) or []):
                    ...
                    lineup_rows.append({
                        **base,
                        ...
                        "shirt_number": _safe_int(p.get("shirt_number") or p.get("number")),
...
if __name__ == "__main__":
    ...
    result = ingest_match_info(args.url, dry_run=args.dry_run, today_only=not args.all_dates)
