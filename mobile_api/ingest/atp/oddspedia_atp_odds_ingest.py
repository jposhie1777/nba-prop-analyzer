...
- Dynamic tournament config (auto-selects active ATP tournament)
...
_repo_root = Path(**file**).resolve().parents[3]
...
print(f"[atp_odds] Tournament : {cfg.tournament_name}")
print(f"[atp_odds] URL        : {target_url}")
print(f"[atp_odds] Slug       : {league_slug}")
print(f"[atp_odds] Season ID  : {season_id}")
...
if insert_only:
    ...
    print(f"[atp_odds] Scraped {len(matches)} matches before tour filter")
    ...
    print(f"[atp_odds] Prepared {len(rows)} BQ rows from {len(matches)} matches")
