...
--
-- Why this version:
-- 1) Uses finished matches even when winner_id is missing (falls back to winner_name).
-- 2) Avoids wiping the target table when source data is temporarily empty.
...
with_flags AS (
  SELECT
    *,
    IF(REGEXP_CONTAINS(COALESCE(score, ''), r'(^|\s)6-[0-4](\s|$)|(^|\s)7-[0-5](\s|$)'), 1, 0) AS has_clean_set,
    IF(REGEXP_CONTAINS(COALESCE(score, ''), r'7-6|6-7'), 1, 0) AS has_tiebreak
  FROM base
),
surface_rollup AS (
  SELECT
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (
        ...
        ORDER BY COALESCE(scheduled_time, TIMESTAMP '1970-01-01') DESC, match_id DESC
      ) AS rn
    FROM with_flags
  )
  WHERE rn <= 20
...
DECLARE staged_rows INT64 DEFAULT (SELECT COUNT(*) FROM atp_player_compare_metrics_stage);

-- Protect existing data: only overwrite destination when stage contains rows.
