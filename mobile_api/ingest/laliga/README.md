# LALIGA ingest DDL vs hard reset

## Which should I run?

- **Normal usage (safe):** run `python -m mobile_api.ingest.laliga.apply_ddl`
  - Creates missing dataset/tables.
  - Reconciles schema to the expected LALIGA ingest shape.
  - Intended for regular backfills and refresh jobs.

- **Break-glass usage (destructive):** run `mobile_api/ingest/laliga/hard_reset.sql`
  - Drops and recreates all LALIGA ingest tables.
...
1. Run `python -m mobile_api.ingest.laliga.apply_ddl`.
2. Retry LALIGA backfill.
3. If the same schema/type error persists, run the hard reset SQL once.
4. Run LALIGA backfill again.
