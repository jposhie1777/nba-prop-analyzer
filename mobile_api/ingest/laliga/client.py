...
BASE_URL = os.getenv("BDL_LALIGA_BASE_URL", "https://api.balldontlie.io/laliga/v1")
API_KEY = os.getenv("BDL_LALIGA_API_KEY") or os.getenv("BALLDONTLIE_API_KEY", "")
TIMEOUT = int(os.getenv("BDL_LALIGA_TIMEOUT_SECONDS", "30"))
RETRY_ATTEMPTS = int(os.getenv("BDL_LALIGA_RETRY_ATTEMPTS", "6"))
RETRY_BASE_SECONDS = float(os.getenv("BDL_LALIGA_RETRY_BASE_SECONDS", "1.0"))
RETRY_CAP_SECONDS = float(os.getenv("BDL_LALIGA_RETRY_CAP_SECONDS", "30.0"))


class LaLigaApiError(RuntimeError):
    pass


def _headers() -> Dict[str, str]:
    if not API_KEY:
        raise LaLigaApiError("BALLDONTLIE_API_KEY is not configured")
...
def _get(path: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        ...
        raise LaLigaApiError(f"{response.status_code} {response.text}")

    raise LaLigaApiError("429 Too many requests, retry budget exhausted")
