...
DROP TABLE IF EXISTS `mls_data.mlssoccer_team_game_stats`;
DROP TABLE IF EXISTS `mls_data.mlssoccer_player_game_stats`;
...
CREATE TABLE `mls_data.mlssoccer_team_game_stats` (
  ingested_at TIMESTAMP NOT NULL,
  season INT64 NOT NULL,
  entity_id STRING,
  payload STRING
...
PARTITION BY DATE(ingested_at)
CLUSTER BY season, entity_id
OPTIONS (description = 'MLS per-club per-match stats from stats-api.mlssoccer.com (entity_id = match_id_club_id)');

CREATE TABLE `mls_data.mlssoccer_player_game_stats` (
  ingested_at TIMESTAMP NOT NULL,
  season INT64 NOT NULL,
  entity_id STRING,
  payload STRING
...
PARTITION BY DATE(ingested_at)
CLUSTER BY season, entity_id
OPTIONS (description = 'MLS per-player per-match stats from stats-api.mlssoccer.com (entity_id = match_id_player_id)');
