...
def _is_doubles_match(match: Dict[str, Any]) -> bool:
    for key in ("match_type", "event_type", "type", "discipline"):
        value = match.get(key)
        if isinstance(value, str) and "double" in value.lower():
            return True

    for key in ("player1", "player2", "player_1", "player_2", "team1", "team2"):
        value = match.get(key)
        if isinstance(value, dict):
            players = value.get("players") or value.get("team") or value.get("members")
...
def build_tournament_bracket_payload(
    *,
    ...
    singles_matches = [match for match in matches if not _is_doubles_match(match)]
    formatted_matches = [_format_match(match) for match in singles_matches]
