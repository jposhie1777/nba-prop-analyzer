...
from ingest.laliga.ingest import ingest_yesterday_refresh, run_full_ingestion

router = APIRouter(tags=["LALIGA"])

LALIGA_MATCHES_TABLE = os.getenv("LALIGA_MATCHES_TABLE", "laliga_data.matches")
LALIGA_STANDINGS_TABLE = os.getenv("LALIGA_STANDINGS_TABLE", "laliga_data.standings")
LALIGA_TEAMS_TABLE = os.getenv("LALIGA_TEAMS_TABLE", "laliga_data.teams")
LALIGA_MATCH_EVENTS_TABLE = os.getenv("LALIGA_MATCH_EVENTS_TABLE", "laliga_data.match_events")
LALIGA_TEAM_MASTER_METRICS_TABLE = os.getenv("LALIGA_TEAM_MASTER_METRICS_TABLE", "laliga_data.team_master_metrics")
...
def _logo_url(team_name: str) -> str:
    ...
    return (
        ...
        f"logos/Spain%20-%20LaLiga/{encoded}.png"
    )
...
@router.post("/ingest/laliga/full")
def ingest_laliga_full(current_season: int = Query(default_factory=_season_default)):
    return run_full_ingestion(current_season=current_season)


@router.post("/ingest/laliga/yesterday-refresh")
def ingest_laliga_yesterday(current_season: int = Query(default_factory=_season_default)):
    return ingest_yesterday_refresh(current_season=current_season)


@router.get("/laliga/moneylines")
def laliga_moneylines(current_season: int = Query(default_factory=_season_default), lookahead_days: int = 7):
    sql = f"""
    WITH latest_matches AS (
      SELECT payload
      FROM `{LALIGA_MATCHES_TABLE}`
      WHERE season = @season
      QUALIFY ROW_NUMBER() OVER (PARTITION BY entity_id ORDER BY ingested_at DESC) = 1
    ),
    latest_standings AS (
      SELECT payload
      FROM `{LALIGA_STANDINGS_TABLE}`
      WHERE season = @season
...
@router.get("/laliga/btts")
def laliga_btts(current_season: int = Query(default_factory=_season_default), lookahead_days: int = 7):
    sql = f"""
    WITH latest_matches AS (
      SELECT payload
      FROM `{LALIGA_MATCHES_TABLE}`
...
@router.get("/laliga/total-goals")
def laliga_total_goals(current_season: int = Query(default_factory=_season_default), lookahead_days: int = 7):
    sql = f"""
    WITH latest_matches AS (
      SELECT payload
      FROM `{LALIGA_MATCHES_TABLE}`
...
@router.get("/laliga/cards")
def laliga_cards(current_season: int = Query(default_factory=_season_default)):
    sql = f"""
    WITH latest_events AS (
      SELECT payload
      FROM `{LALIGA_MATCH_EVENTS_TABLE}`
      WHERE season = @season
      QUALIFY ROW_NUMBER() OVER (PARTITION BY entity_id ORDER BY ingested_at DESC) = 1
    ),
    teams AS (
...
@router.get("/laliga/upcoming-today")
def laliga_upcoming_today(current_season: int = Query(default_factory=_season_default)):
    sql = f"""
    WITH latest_matches AS (
      SELECT payload
      FROM `{LALIGA_MATCHES_TABLE}`
...
@router.get("/laliga/team-master-metrics")
def laliga_team_master_metrics():
    sql = f"""
    SELECT
      team_id,
      team_name,
