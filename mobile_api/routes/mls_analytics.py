...
MLS_MATCHES_TABLE = os.getenv("MLS_MATCHES_TABLE", "mls_data.matches")
MLS_STANDINGS_TABLE = os.getenv("MLS_STANDINGS_TABLE", "mls_data.standings")
MLS_TEAMS_TABLE = os.getenv("MLS_TEAMS_TABLE", "mls_data.teams")
MLS_MATCH_EVENTS_TABLE = os.getenv("MLS_MATCH_EVENTS_TABLE", "mls_data.match_events")
MLS_TEAM_MASTER_METRICS_TABLE = os.getenv("MLS_TEAM_MASTER_METRICS_TABLE", "mls_data.team_master_metrics")
...
def _logo_url(team_name: str) -> str:
    ...
    return (
        ...
        f"logos/USA%20-%20MLS/{encoded}.png"
    )
...
@router.post("/ingest/mls/full")
def ingest_mls_full(current_season: int = Query(default_factory=_season_default)):
...
def mls_moneylines(current_season: int = Query(default_factory=_season_default), lookahead_days: int = 7):
    sql = f"""
    WITH latest_matches AS (
      SELECT payload
      FROM `{MLS_MATCHES_TABLE}`
      WHERE season = @season
      QUALIFY ROW_NUMBER() OVER (PARTITION BY entity_id ORDER BY ingested_at DESC) = 1
    ),
    latest_standings AS (
      SELECT payload
      FROM `{MLS_STANDINGS_TABLE}`
      WHERE season = @season
...
@router.get("/mls/btts")
def mls_btts(current_season: int = Query(default_factory=_season_default), lookahead_days: int = 7):
    sql = f"""
    WITH latest_matches AS (
      SELECT payload
      FROM `{MLS_MATCHES_TABLE}`
...
@router.get("/mls/total-goals")
def mls_total_goals(current_season: int = Query(default_factory=_season_default), lookahead_days: int = 7):
    sql = f"""
    WITH latest_matches AS (
      SELECT payload
      FROM `{MLS_MATCHES_TABLE}`
...
@router.get("/mls/cards")
def mls_cards(current_season: int = Query(default_factory=_season_default)):
    sql = f"""
    WITH latest_events AS (
      SELECT payload
      FROM `{MLS_MATCH_EVENTS_TABLE}`
      WHERE season = @season
      QUALIFY ROW_NUMBER() OVER (PARTITION BY entity_id ORDER BY ingested_at DESC) = 1
    ),
    teams AS (
...
@router.get("/mls/upcoming-today")
def mls_upcoming_today(current_season: int = Query(default_factory=_season_default)):
    sql = f"""
    WITH latest_matches AS (
      SELECT payload
      FROM `{MLS_MATCHES_TABLE}`
...
def mls_standings():
    ...
    FROM `{MLS_TEAM_MASTER_METRICS_TABLE}`
...
@router.get("/mls/team-master-metrics")
def mls_team_master_metrics():
    sql = f"""
    SELECT
      team_id,
      team_name,
