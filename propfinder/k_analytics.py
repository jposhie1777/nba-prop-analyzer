...
def calibrate_weights(days=30):
    """
    The learning engine. Adjusts weights based on factor correlations
    with actual hit outcomes.

    Strategy:
    - Factors with positive correlation to hits → boost weight
    ...
    """
    log.info("Starting weight calibration with %s-day lookback", days)
    ...
    # Get per-factor hit rate differentials
    sql = f"""
    WITH picks AS (
      SELECT *
      FROM {TABLE}.k_picks_daily`
    ...
    if weight_rows:
        ...
        if errors:
            ...
            log.info("Wrote %s calibrated weights to k_model_weights", len(weight_rows))
...
def main():
    ...
    log.info("Step 2: Calibrating weights...")
