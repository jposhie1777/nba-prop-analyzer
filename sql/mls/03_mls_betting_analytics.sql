-- ============================================================
-- 03_mls_betting_analytics.sql
--
-- Builds the final MLS betting analytics table.
-- Joins FanDuel markets + deep links with team form metrics.
--
-- Sources:
--   soccer_data.raw_fanduel_soccer_markets  → odds + deep links
--   soccer_data.mls_team_name_map           → FD name → team_id (step 1)
--   soccer_data.mls_team_form               → form metrics (step 2)
--   soccer_data.odds_lines                  → multi-book consensus
-- Output:
--   soccer_data.mls_betting_analytics
-- ============================================================

CREATE OR REPLACE TABLE `graphite-flare-477419-h7.soccer_data.mls_betting_analytics`
...
-- ── Step 1: Deduplicate FanDuel MLS markets ───────────────────────────────
fd_raw AS (
  SELECT * EXCEPT(rn)
  FROM (
    SELECT *,
...
SELECT
  ...
  -- Multi-book consensus (NULLs where market not in odds_lines)
  c.books_reporting,
  ...
  -- Totals line edge (only for totals markets)
  CASE
...
LEFT JOIN `graphite-flare-477419-h7.soccer_data.mls_team_name_map` hmap
  ON f.home_team = hmap.fd_name
LEFT JOIN `graphite-flare-477419-h7.soccer_data.mls_team_name_map` amap
  ON f.away_team = amap.fd_name

-- Form metrics
LEFT JOIN `graphite-flare-477419-h7.soccer_data.mls_team_form` h
  ON hmap.team_id = h.team_id
LEFT JOIN `graphite-flare-477419-h7.soccer_data.mls_team_form` a
